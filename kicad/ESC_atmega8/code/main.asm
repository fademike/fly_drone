...
.IF DAMPED_MODE_ENABLE == 1
.EQU	DEFAULT_PGM_MULTI_PWM_FREQ	 	= 3;3 	; 1=High 		2=Low 		3=DampedLight 
.ELSE
...
.EQU	DEFAULT_PGM_PPM_MIN_THROTTLE		= 1;37	; 4*37+1000=1148
.EQU	DEFAULT_PGM_PPM_MAX_THROTTLE		= 50;208; 4*208+1000=1832
.EQU	DEFAULT_PGM_PPM_CENTER_THROTTLE	= 20;122; 4*122+1000=1488 (used in bidirectional mode)
...
.EQU	RCP_TIMEOUT_PPM	=	5;10	; Number of timer2H overflows (about 32ms) before considering rc pulse lost
.EQU	RCP_TIMEOUT		= 	12;24	; Number of timer2L overflows (about 128us) before considering rc pulse lost
.EQU	RCP_SKIP_RATE		= 	3;6	; Number of timer2L overflows (about 128us) before reenabling rc pulse detection
.EQU	RCP_MIN			= 	0	; This is minimum RC pulse length
.EQU	RCP_MAX			= 	255	; This is maximum RC pulse length
.EQU	RCP_VALIDATE		= 	2	; Require minimum this pulse length to validate RC pulse
.EQU	RCP_STOP			= 	1	; Stop motor at or below this pulse length
...
.EQU	PWM_START			= 	1;50 	; PWM used as max power during start
...
MY_CNT_SW:				.BYTE	1		; My count
MY_CNT_SW1:				.BYTE	1		; My count
MY_CNT_SW2:				.BYTE	1		; My count
MY_CNT_SW3:				.BYTE	1		; My count
...
t0_int_ppm_timeout_set:
	...
	;LDI 	Temp1, New_Rcp		; загоняем в регистр код буквы «E»
	;OUT	UDR, I_Temp1	; шлем байт

	sts	Rcp_Stop_Cnt, Zero			; Reset rcp stop counter																										//uart send
...
rcp_int_second_meas_pwm_freq:
	...
	cpi	I_Temp1, low(70);140)			; If pulse below 70us, not accepted																							//TODO
	cpc	I_Temp2, Zero
...
rcp_int_check_12kHz:
	...
	cpi	I_Temp1, low(100);200)			; If below 100us, 12kHz pwm is assumed																						//TODO
	cpc	I_Temp2, Zero
...
rcp_int_check_8kHz:
	...
	cpi	I_Temp1, low(180);360)			; If below 180us, 8kHz pwm is assumed																						//TODO
	ldi	XL, high(360)
	cpc	I_Temp2, XL
...
rcp_int_check_4kHz:
	...
	cpi	I_Temp1, low(360);720)			; If below 360us, 4kHz pwm is assumed																						//TODO
	ldi	XL, high(720)
	cpc	I_Temp2, XL
...
rcp_int_check_2kHz:
	...
	cpi	I_Temp1, low(720);1440)			; If below 720us, 2kHz pwm is assumed																						//TODO
	ldi	XL, high(1440)
	cpc	I_Temp2, XL
...
rcp_int_check_1kHz:
	...
	cpi	I_Temp1, low(1100);2200)			; If below 1100us, 1kHz pwm is assumed																					//TODO
	ldi	XL, high(2200)
	cpc	I_Temp2, XL
...
rcp_int_ppm_calculate:
	ldi	XL, 125;250						; Add 1000us to minimum																									//TODO
	add	I_Temp3, XL
	mov	XL, Zero
	adc	XL, Zero
...
.ENDIF
	...
	;OUT	UDR, I_Temp1	; шлем байт																														;send then wait
...
rcp_int_ppm_max_checked:
	...
	;ldi	I_Temp1, Prev_Comm_L																																						; DELETE
	;OUT	UDR, I_Temp1	; шлем байт	

	;ldi	I_Temp1, MY_CNT_SW	
	;sts MY_CNT_SW, Zero																																								; DELETE
	;OUT	UDR, I_Temp1	; шлем байт	
	...
	rol	I_Temp1
	ldi	I_Temp2, 0
	;OUT	UDR, I_Temp1	; шлем байт																							; real position Rcp
...
rcp_int_limited:
	...
	sts	New_Rcp, I_Temp1			; Store new pulse length
	;OUT	UDR, I_Temp1																														;Set New_Rcp
	lds	I_Temp2, MY_CNT_SW1		
	sbrs 
	
																													;Set New_Rcp
	lds	I_Temp2, MY_CNT_SW	
	sts MY_CNT_SW, Zero																																								; DELETE
	OUT	UDR, I_Temp2	; шлем байт	
...
beep_f1:	; Entry point 1, load beeper frequency 1 settings
	ldi	Temp3, 29;58		; Off wait loop length
	ldi	Temp4, 60;120	; Number of beep pulses
	rjmp	beep
...
calc_next_comm_timing:			; Entry point for run phase
	...
	sts	Prev_Comm_H, Temp2
																																												; DELETE
	;OUT	UDR, Temp1	; шлем байт	
	;sbi PORTD,1
	;cbi PORTD,1
	;sbic PIND,1	//Пропустить следующую команду если бит PB1 регистра PINB = 
	;	cbi PORTD,1	//Гасим светодиод
	;sbis PIND,1	//Пропустить следующую команду если бит PB1 регистра PINB = 1
	;	sbi PORTD,1	//Зажигаем светодиод
...
; Internal Hardware Init  ======================================
		.equ 	XTAL111 = 8000000 	
		.equ 	baudrate111 = 19200;9600  
		.equ 	bauddivider111 = XTAL111/(16*baudrate111)-1
 
 
uart_init:	LDI 	Temp3, low(bauddivider111)
		OUT 	UBRRL,Temp3
		LDI 	Temp3, high(bauddivider111)
		OUT 	UBRRH,Temp3
...
; Прерывания запрещены, прием-передача разрешен.
		LDI 	Temp3, (1<<RXEN)|(1<<TXEN)|(0<<RXCIE)|(0<<TXCIE)|(0<<UDRIE)
		OUT 	UCSRB, Temp3	
 
; Формат кадра - 8 бит, пишем в регистр UCSRC, за это отвечает бит селектор
		LDI 	Temp3, (1<<URSEL)|(1<<UCSZ0)|(1<<UCSZ1)
		OUT 	UCSRC, Temp3

uart_snt:	SBIS 	UCSRA,UDRE	; Пропуск если нет флага готовности
		RJMP	uart_snt 	; ждем готовности - флага UDRE
 
		OUT	UDR, Temp3	; шлем байт
		RET			; Возврат
; my program end

uart_ssnt:	LDI 	Temp3,'0'		; загоняем в регистр код буквы «E»
		OUT	UDR, Temp3	; шлем байт
		RET			; Возврат
...
;		RCALL 	uart_init 	; вызываем нашу процедуру инициализации.
		...
		;LDI 	Temp3,'1'		; загоняем в регистр код буквы «E»
		;RCALL	uart_snt	; Вызываем процедуру отправки байта. 
 
		;LDI 	Temp3,'2'		; загоняем в регистр код буквы «E»
		;RCALL	uart_snt	; Вызываем процедуру отправки байта. 
		;LDI 	Temp3,'3'		; загоняем в регистр код буквы «E»
		;RCALL	uart_snt	; Вызываем процедуру отправки байта. 
;		NOP			; Любой дальнейший код
;		NOP
;		NOP

	LDI 	Temp3, low(bauddivider111)
		OUT 	UBRRL,Temp3
		LDI 	Temp3, high(bauddivider111)
		OUT 	UBRRH,Temp3
...
arm_end_beep:
	...
	cli						; Disable all interrupts
	;xcall beep_f4				; Signal that rcpulse is ready																	; Signal that rcpulse is ready	
	;xcall beep_f4
	;xcall beep_f4
	sei						; Enable all interrupts
	xcall wait200ms
...
;		 evaluate_comparator_integrity	; Check whether comparator reading has been normal
	...
	lds	Temp1, MY_CNT_SW																																			; Periud detect
	sbrs Temp1, 7
	inc Temp1
	sts	MY_CNT_SW, Temp1
